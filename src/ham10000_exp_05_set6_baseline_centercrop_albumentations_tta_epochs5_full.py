...
if __name__ == '__main__':
    ...
    trainer = Ham10000ResNet18Trainer(train_dataloader, epochs=5)
    ...
    trainer.run_training_and_validation(model, writer, validation_dataloader)
    log_time('\tTraining end time:')

    print('4 - validate model (TTA)')
    validator = Ham10000ResNet18Validator(validation_dataloader)
    model.eval()
    validator.run_tta_validation(model)
